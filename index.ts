...
for (const ui of uis) {
  ...
  for (const checkpoint of checkpoints as Checkpoint[]) {
    ...
    pvs = pvs.replace(
      ...
      `CHECKPOINT_MODELS=(\n    '${url(`${repositories[checkpoint.base]}/CHECKPOINT/${checkpoint.filename}`)}'\n)`,
    );
    ...
    const embeddingsPositive = embeddings
      .filter((embedding) => embedding.base.includes(checkpoint.base) && embedding.type === 'positive')
    ...
    const embeddingsNegative = embeddings
      .filter((embedding) => embedding.base.includes(checkpoint.base) && embedding.type === 'negative')
      .map((embedding) => embedding.filename);
    ...
    if (ui.id === 'comfy' || ui.id === 'forge') {
      pvs = pvs.replace(
        ...
        `provisioning_get_models \\\n        "\${WORKSPACE}/storage/stable_diffusion/${ui.id === 'comfy' ? 'models/' : ''}embeddings/positive" \\\n        "\${EMBEDDINGS_POSITIVE[@]}"\n    provisioning_get_models \\`,
      );
      pvs = pvs.replace(
        /provisioning_get_models \\/m,
        `provisioning_get_models \\\n        "\${WORKSPACE}/storage/stable_diffusion/${ui.id === 'comfy' ? 'models/' : ''}embeddings/negative" \\\n        "\${EMBEDDINGS_NEGATIVE[@]}"\n    provisioning_get_models \\`,
      );
    }
